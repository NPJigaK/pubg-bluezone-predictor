...
このツールは、PLAYERUNKNOWN'S BATTLEGROUNDSの安置予測練習を目的としたPC向けWebアプリケーションです。

[こちら](https://kagijpn.github.io/pubg-bluezone-predictor/top/)からツールを利用できます。

推奨ブラウザはGoogle Chromeです。**AdBlock**系のブラウザ拡張機能を導入していると、正常に動作しない可能性があります。

初めてご利用になる方は、以下の[利用方法](#利用方法)をお読みください。

## 利用方法
あらかじめ用意されている試合データを利用することも、自分で練習用の試合データを作成することも可能です。

- [あらかじめ用意されている試合データを使う](#あらかじめ用意されている試合データを使って利用する場合)
- [自分自身で練習用の試合データを作る](#自分自身で練習用の試合データを作る場合)

### あらかじめ用意されている試合データを使って利用する場合
本ツールであらかじめ用意している試合データがいくつかあります。
...
**GET YOUR OWN API KEY**というところを押して、表示される指示に従って進めていき、会員登録（無料）をします。（英語のサイトですが、特に難しい操作はありません）
...
### tips
このツールでは、API Keyを複数登録することが可能です。[PUBG Developer Portal](https://developer.pubg.com/)のアカウント1つで最大5つのAPI Keyを発行することができます。5つすべてを発行して登録することを推奨します。

#### 試合検索ページの使い方
[試合検索ページ](https://kagijpn.github.io/pubg-bluezone-predictor/players/)にアクセスすると、以下のような画面が表示されます。
...
#### 試合一覧ページの使い方
試合検索ページで正しく入力された情報に基づいて遷移すると、以下のような画面が表示されます。
...
### tips
- 最終安置に到達しなくても、他の試合に切り替えることができます。
...
## ライセンス
このプロジェクトは [Apache License 2.0](https://github.com/KagiJPN/pubg-bluezone-predictor/blob/master/LICENSE) の下でライセンスされています。
